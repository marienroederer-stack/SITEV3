...
Différences avec le format actuel (voir importer.py) :
 ...
 - pas de Durée Totale / Annonce / File : l'attente moyenne globale ne peut donc pas être
   calculée correctement pour cette période (les colonnes sources n'existent pas) — laissée
   à la seule sonnerie plutôt qu'approximée, pour ne pas produire un chiffre plausible mais
   faux. Les ratios d'appels >3/4/5/6 min, eux, restent fiables : ils se basent sur la
   colonne Comm (durée de communication réelle), présente dans ce format. L'attente sur
   sonnerie et l'opérateur (colonne Identifiant) restent également fiables et individualisés
   normalement.
"""
